...
| 层次 | 做法 | 结果 |
| --- | --- | --- |
| 单测 | 编码器（含 405 音节全量回归）、提示策略、统计、历史、题库与语料质量、出题器、状态机、文档写法 | `npm test` **85/85** |
| 交叉验证 | 下载**小鹤官方码表**，对每个单字取官方编码前两位与编码器输出逐字比对 | **7674/7701 一致（99.649%）** |
| 端到端 | 用无头浏览器在真 `file://` 下跑完整流程（渲染、按键、暂停、刷新持久化、按需加载） | **19/19** |

那 27 处不一致经解码确认**全部是生僻多音字的读音取舍差异**
（例如 蔈：官方 `bn`→biao，编码器 `pn`→piao），不是编码错误。
