...
暂停期间**不接收输入、不计入用时、卡住提示也不会触发**。

最后一条不是细节：没有暂停功能时，你走开几分钟，键位图会在 3 秒时自动闪提示，
于是那道题被标记成「依赖提示」，回来之后它已经不算你的独立成绩了，
可你根本没看到那个提示。现在这种情况会走「离开较久 → 自动暂停」，
继续时**本题重新开始**。

### 卡住自动提示

一个键想不出来、干等 **3 秒**，键位图上就会闪烁「该按的那个键」，一旦敲键就重新计时。
默认**只闪键位、不给答案**（想直接看答案按 `Tab`，或在「更多设置」里打开「6 秒后显示答案」）。

提示过的题**不计入独立正确率**，会单独统计「依赖提示」——所以卡住时放心看提示，
成绩不会虚高，也不会看不出自己的真实水平。

## 统计与复盘

- **结束本轮并结算**：成绩单——速度、独立正确率、按键正确率、最长连击，以及
...
| 层次 | 做法 | 结果 |
| --- | --- | --- |
| 单测 | 手写 ~90 条权威用例 + 405 音节全量回归 + 提示策略 + 统计 + 历史 + 题库质量 + 出题器 + 状态机 | `npm test` **80/80** |
...
docs/DESIGN.md              # 设计文档与实施记录（含每个决策的取舍与踩过的坑）
```
...
**词库怎么筛的**：jieba 词典有 58 万条，直接拿来用会混进大量没练习价值的词。三道过滤——
按词性剔除人名/地名/机构/专名（否则会练到「云中鹤」「索马里」）、用 opencc 去掉繁体重复项、
要求每个字都在常用字表里；成语单独放宽词频门槛并留了 1200 个名额。

**语料按覆盖率补，不按条数堆**：`npm run gen:corpus` 会打印
「前 1000 常用字里还没覆盖的有哪些」，照着那个清单写句子。
最初 65 条语料只覆盖了前 500 常用字的 **37%**（也就是说最常用的字里有六成在句子模式里练不到），
照清单扩到 335 条后是 **98% / 71%**，并且加了测试锁住这个下限。
