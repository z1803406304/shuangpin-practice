...
/**
 * 各版本相对上一版改了什么。发新版时在这里加一段，Release 说明会自动带上。
 * 写法：'v1.1.0': `- 改了 A\n- 修了 B`
 */
const CHANGELOG = {
  'v1.1.0': `- **句子 / 短文语料从 65 条扩到 335 条**，并改用「常用字覆盖率」驱动扩充：
  前 500 常用字覆盖从 37% 提升到 **98%**，前 1000 从 24% 到 **71%**。
  原来最常用的字里有六成在句子模式里根本练不到 —— 条数少是表象，覆盖面窄才是问题。
- 语料新增「只用常用字」约束（生僻字会被生成脚本拒绝），并加了覆盖率回归测试
  （前 500 低于 95%、前 1000 低于 68% 会让 CI 失败）
- 单行长度上限 26 → 36 字，短文更自然
- 补充 Windows / PowerShell 的执行策略说明：默认策略会阻止 \`npm\` 脚本，用 \`npm.cmd\` 或放开策略`,
}

/**
 * 每个版本都会带的长期内容（功能清单 / 用法 / 正确性 / 数据来源 / 许可证）。
 */
const EVERGREEN_NOTES = `## 主要特性
...
/** 拼装成最终 Release 说明：本版更新（如果有）+ 长期内容 */
const NOTES = [CHANGELOG[TAG_NORMALIZED] ? `## 本版更新（${TAG_NORMALIZED}）\n\n${CHANGELOG[TAG_NORMALIZED]}` : '', EVERGREEN_NOTES]
  .filter(Boolean)
  .join('\n\n')
